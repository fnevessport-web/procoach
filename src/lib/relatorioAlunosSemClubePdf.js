...
// PDF de conferência — lista curada (o coordenador seleciona manualmente na tela antes de
// exportar, tirando cortesia não marcada como tal e nome que sabe estar errado) de alunos
// com aula dada no período que não aparecem na planilha de pagamento que o clube manda.
// Traz nível/turma (individual ou grupo, já que é isso que define o valor da mensalidade,
// ver estimarMensalidadeTenis em constants/modalidades.js) e o valor estimado de 1
// mensalidade por aluno, pro coordenador ter noção de quanto pode estar em jogo — cálculo
// interno nosso (tabela de mensalidade do ProCoach), não confirmado com o clube, por isso
...
export async function exportarAlunosSemClubePDF(selecionados, { periodo }) {
  ...
  empresas.forEach(empresa => {
    ...
    const totalEmpresa = itens.reduce((s, i) => s + (i.valorEstimadoMensalidade || 0), 0)
    ...
    autoTable(doc, {
      startY: cursorY,
      head: [['Aluno', 'Modalidade', 'Turma', 'Dia(s)', 'Horário', 'Valor estimado']],
      body: itens.map(i => [i.nome, i.modalidade, i.nivel || '—', i.diasSemanaLabel, i.horario, i.valorEstimadoMensalidade != null ? fmtBRL(i.valorEstimadoMensalidade) : '—']),
      theme: 'plain',
