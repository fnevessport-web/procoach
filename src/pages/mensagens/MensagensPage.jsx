  ...
  return (
    <div className="fade-in">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '16px 0 18px' }}>
        <div>
          <h1 style={{ fontSize: '20px', fontWeight: '700', color: 'var(--color-text-dark-primary)', margin: 0 }}>Mensagens</h1>
          {podeVerInboxGeral && (
            <p style={{ fontSize: '11px', color: 'var(--color-text-dark-secondary)', margin: '2px 0 0' }}>Inbox geral — todas as conversas</p>
          )}
        </div>
      ...
      {isLoading ? <Loading /> : conversas.length === 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '64px 24px', textAlign: 'center' }}>
          <MessageCircle size={40} color="var(--color-action-primary)" style={{ opacity: 0.2, marginBottom: '14px' }} />
          <p style={{ fontSize: '13px', color: 'var(--color-text-dark-secondary)', margin: '0 0 4px', fontWeight: '600' }}>Nenhuma conversa ainda</p>
          <p style={{ fontSize: '12px', color: 'var(--color-text-dark-secondary)', margin: 0 }}>Toque em + pra falar com alguém do time.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {conversas.map(c => (
            <button key={c.id} onClick={() => onAbrirConversa(c.id)} style={{
              ...
              borderRadius: '12px', border: '0.5px solid var(--color-border-dark)', backgroundColor: 'var(--color-surface-dark-raised)',
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--color-text-dark-primary)' }}>{c.titulo}</div>
                <div style={{ fontSize: '12px', color: 'var(--color-text-dark-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {c.ultimaMensagem || 'Sem mensagens ainda'}
                </div>
              </div>
              <span style={{ fontSize: '10px', color: 'var(--color-text-dark-muted)', flexShrink: 0 }}>{formatarHora(c.ultimaData)}</span>
            </button>
          ))}
        </div>
      )}
  ...
  return (
    <div className="fade-in">
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', margin: '16px 0 18px' }}>
        <button onClick={onFechar} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-action-primary)', padding: '4px' }}>
          <ChevronLeft size={22} />
        </button>
        <h1 style={{ fontSize: '17px', fontWeight: '700', color: 'var(--color-text-dark-primary)', margin: 0 }}>Nova conversa</h1>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {contatos.map(c => (
            <button key={c.user_id} disabled={abrirConversa.isPending} onClick={() => selecionar(c)} style={{
              ...
              <div>
                <div style={{ fontSize: '13px', fontWeight: '600', color: 'var(--color-text-dark-primary)' }}>{c.nome}</div>
                <div style={{ fontSize: '11px', color: 'var(--color-text-dark-secondary)', textTransform: 'capitalize' }}>{c.role}</div>
              </div>
            </button>
          ))}
        </div>
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', margin: '16px 0 12px', flexShrink: 0 }}>
        <button onClick={onVoltar} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-action-primary)', padding: '4px' }}>
          <ChevronLeft size={22} />
        </button>
        <h1 style={{ fontSize: '15px', fontWeight: '700', color: 'var(--color-text-dark-primary)', margin: 0 }}>Conversa</h1>
      </div>
        ...
        {isLoading ? <Loading /> : mensagens.map(m => {
          ...
          return (
            <div key={m.id} style={{ display: 'flex', flexDirection: 'column', alignItems: minha ? 'flex-end' : 'flex-start' }}>
              {!minha && <span style={{ fontSize: '10px', color: 'var(--color-text-dark-secondary)', margin: '0 4px 2px' }}>{m.autorNome}</span>}
              <div style={{
                maxWidth: '75%', padding: '9px 12px', borderRadius: minha ? '14px 14px 2px 14px' : '14px 14px 14px 2px',
                background: minha ? 'var(--color-action-primary)' : 'var(--color-surface-dark-raised)',
                border: minha ? 'none' : '0.5px solid var(--color-border-dark)',
                color: minha ? 'white' : 'var(--color-text-dark-primary)', fontSize: '13px', lineHeight: '1.4',
                wordBreak: 'break-word',
              }}>
                {m.texto}
              </div>
              <span style={{ fontSize: '9px', color: 'var(--color-text-dark-muted)', margin: '2px 4px 0' }}>{formatarHora(m.criado_em)}</span>
            </div>
          )
        })}
        <div ref={fimRef} />
      ...
      <div style={{ display: 'flex', gap: '8px', paddingTop: '8px', flexShrink: 0 }}>
        <input
          ...
          style={{
            flex: 1, padding: '10px 14px', borderRadius: '20px', backgroundColor: 'var(--color-surface-dark-raised)',
            border: '1px solid var(--color-border-dark)', color: 'var(--color-text-dark-primary)', fontSize: '13px', outline: 'none', boxSizing: 'border-box',
          }}
        />
