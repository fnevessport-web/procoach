...
3. **Chrome fixo do app** (`Header.jsx`, `Sidebar.jsx`, `BottomNav.jsx`, `SinoAlertas.jsx`): fica
   ...
   troca de contexto (é a barra de navegação, não conteúdo). Chegou a existir uma variação em
   marrom-café (tokens `--color-chrome-*`, inspirada no app "Lits") — revertida a pedido, o
   chrome ficou no verde-court de sempre. O `Sidebar.jsx` (`.sidebar-desktop` no CSS) é
   translúcido com blur ("espelhado fosco", `backdrop-filter`), diferente do Header/BottomNav
   que são opacos (Header também usa blur, mas com bg quase opaco a 85%) — efeito pedido
   explicitamente, não usar `background-color` sólido nele.
